...
from models.vgg.vgg_ import VGG_, vgg16_
import dataset.dataset_cub200
#import kmeans

##############################################################################################################
class GroupingFineTuner_VGG:
...
def get_args():
    ...
    parser.add_argument('--arch', '--a', default='VGG', help='model architecture: (default: VGG)')
...
if __name__ == '__main__':
    ...
    model = torch.load("model_training_m20").cuda()
    model = model.module.state_dict()
    model_ = vgg16_()
    model['classifier.6.weight'] = model_.state_dict()['classifier.6.weight'] 
    model['classifier.6.bias'] = model_.state_dict()['classifier.6.bias']
    model['classifier.6._mask'] = model_.state_dict()['classifier.6._mask']
    model_.load_state_dict(model)
    print("model:", model_)

    #model = torch.nn.DataParallel(model).cuda()

    fine_tuner = GroupingFineTuner_VGG(args.train_path, args.test_path, model_)

    if args.train:
        fine_tuner.train(epoches = args.epochs)
        torch.save(model_, "model_training_final")
        print("model_training_final:", model_)
