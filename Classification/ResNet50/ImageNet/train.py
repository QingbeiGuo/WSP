...
import math
#model
from models.resnet.resnet import ResNet, resnet50
#dataset
...
##############################################################################################################
	...
	def prune(self):
		...
		for k, conv_item in enumerate(list(self.conv_fc_candidate_idx)):
		    ...
		    print("item-conv_fc2", conv_item)
		    #mask pruning
		    self.model.set_conv_linear_mask(i+1, 0, conv_item, conv_item)
...
#    parser.add_argument('--weight_decay', '--wd', type=float, default=0, help='weight decay (default: 1e-4)')
    parser.add_argument('--conv_compress_percentage', '--conv_compress_percentage', type=int, default=70, help = 'compress percentage')
    parser.add_argument('--fc_compress_percentage', '--fc_compress_percentage', type=int, default=70, help = 'compress percentage')
    parser.add_argument('--conv_fc_compress_percentage', '--conv_fc_compress_percentage', type=int, default=70, help = 'compress percentage')
...
#	os.environ["CUDA_VISIBLE_DEVICES"] = "1,2,3"
	...
	torch.save(model.state_dict(), "model_training_m.pth")'''
	...
	model = torch.load("model_compress_40").cuda()
	print("model_training:", model)
	...
	if args.compress:
	    fine_tuner.compress()
	    torch.save(model, "model_compress_0")
	elif args.train:
